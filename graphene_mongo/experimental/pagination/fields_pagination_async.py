...
from ...utils import get_query_fields, has_page_info, sync_to_async
...
class AsyncMongoenginePaginationField(AsyncMongoengineConnectionField):
    ...
    def type(self):
        ...
        ), "AsyncMongoenginePaginationField only accepts AsyncMongoenginePaginationObjectType types"
    ...
    async def default_resolver(self, _root, info, required_fields=None, resolved=None, **args):
        ...
        if resolved is not None:
            ...
            if isinstance(items, QuerySet):
                try:
                    if last is not None or requires_page_count:  # Pagination Logic
        ...
        if not bool(args) or not is_partial:
            if isinstance(self.model, mongoengine.Document) or isinstance(
                ...
                connection_fields = [
                    ...
                    if isinstance(
                        self.fields[field],
                        AsyncMongoenginePaginationField,  # Pagination Logic
                    )
                ]
