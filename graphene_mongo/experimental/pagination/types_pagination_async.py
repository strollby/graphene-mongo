...
from graphene_mongo.experimental.pagination.graphene_ import PageConnection
from graphene_mongo.registry import Registry, get_global_async_registry, get_inputs_async_registry
from graphene_mongo.types import construct_fields, construct_self_referenced_fields
...
from .fields_pagination_async import AsyncMongoenginePaginationField


def create_graphene_generic_class_async(object_type, option_type):
    class AsyncMongoenginePaginationGenericObjectTypeOptions(option_type):
        model = None
    ...
    class AsyncGrapheneMongoenginePaginationGenericType(object_type):
        @classmethod
        def __init_subclass_with_meta__(
            ...
            **options,
            ...
            if connection is not None:
                assert issubclass(connection, PageConnection), (
                    "The attribute connection in {}.Meta must be of type PageConnection. "
                    'Received "{}" instead.'
            ...
            if connection_field_class is not None:
                ...
                connection_field_class = AsyncMongoenginePaginationField

            if _meta:
                assert isinstance(_meta, AsyncMongoenginePaginationGenericObjectTypeOptions), (
                    "_meta must be an instance of AsyncMongoenginePaginationGenericObjectTypeOptions, "
                    "received {}"
                ).format(_meta.__class__)
            else:
                _meta = AsyncMongoenginePaginationGenericObjectTypeOptions(option_type)
            ...
            super(AsyncGrapheneMongoenginePaginationGenericType, cls).__init_subclass_with_meta__(
    ...
    return (
        AsyncGrapheneMongoenginePaginationGenericType,
...
(
    ...
    AsyncMongoenginePaginationObjectTypeOptions,
) = create_graphene_generic_class_async(ObjectType, ObjectTypeOptions)

(
    AsyncMongoenginePaginationInterfaceType,
    MongoengineInterfacePaginationTypeOptions,
